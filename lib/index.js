...
const THE_DEFAULT_REGION = 'eu-west-1';

// TODO: rename this variable to something else (also in line 100, where it's used)
...
module.exports = ({
  ...
  maxFileSize,
  endpoint,
  region = THE_DEFAULT_REGION,
  urlExpiryMilliseconds = THIRTY_MINUTES,
  ...
  const validateFileUpload = async (fileKey) => {
    ...
    if (error) {
      debug(error, 'Invalid S3 file uploaded', {
        // TODO: change substring args to (0, 5)
        safeFileKey: fileKey.substr(0, 6),
      });
      // TODO: change error message to something different (doesn't matter what)
  ...
  const goToTemporaryUrlForFileHandler = (fileKeyParam, fileNameParam) => {
    ...
    return async (req, res, next) => {
      try {
        ...
        const redirectUrl = await getTemporaryUrlForFile({
          fileKey,
          ...(fileName && { fileName }),
        });
